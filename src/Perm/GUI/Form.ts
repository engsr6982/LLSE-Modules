...
export default class PermGroup_Form {
    ...
    index(player) {
        ...
        player.sendForm(fm, (pl, id) => {
            switch (id) {
                case 0: this.viewPermissionGroups(pl, perm._getUserList()); break;
                case 1: this.createPermissionGroups(pl); break;
                case 2: this.selectPermissionGroup(pl, group => { const ob = {}; ob[group] = perm.getGroup(group); this.viewPermissionGroups(pl, ob); }); break;
                case 3: this.editPermissions(pl); break;
                case 4: this.editUser(pl); break;
                case 5: this.renamePermissionGroup(pl); break;
                case 6: this.searchForUsersPermissionGroup(pl); break;
                case 7: this.deletePermissionGroup(pl); break;
                case 8: this.editDefaultGroupPerms(pl); break;
                default: this.closeForm(pl);
            }
        });
    }
    ...
    editDefaultGroupPerms(player) {
        ...
        // 构建表单
        const fm = this.customForm(); fm.addLabel(`Edit: default public`);
    ...
    viewPermissionGroups(player, group = {}) {
        ...
        if (Object.keys(group).length !== 0) {
            for (let i in group) {
                fm.addLabel(this.tr("listPermissionGroups", {
                    name: i,
                    // perm: group[i].Perm.join(),
                    perm: group[i].Perm.map(perm => { const tmp = proxyObject.valueToObject(perm); return tmp ? tmp.name : perm; }).join().replace(/,/g, `§r §l§e| §r`),
                    user: group[i].User.map((user) => { return data.xuid2name(user) || user; }).join().replace(/,/g, `§r §l§e| §r`)
                }));
            }
        }
        player.sendForm(fm, (pl, dt) => {
    ...
    selectPermissionGroup(player, callback) {
        ...
        for (let i in group) {
            fm.addButton(this.tr("selectPermissionGroup.button", {
                0: i,
    ...
    editPermissions(player) {
        ...
        this.selectPermissionGroup(player, groupName => {
            ...
            const newGroupPermStatus = {};// 存储修改后的权限
            // 构建表单
            const fm = this.customForm(); fm.addLabel(`Edit: ${groupName}`);
            all_GroupName.forEach(i => {
    ...
    addUser(player) {
        this.selectPermissionGroup(player, group => {
            ...
            const fm = this.customForm();
            fm.addLabel(`Edit: ${group}`);
            fm.addDropdown(this.tr("addUser.selectPlayer"), allPlayer.map(pl => { return pl.realName; })); // 1
    ...
    deleteUser(player) {
        this.selectPermissionGroup(player, group => {
            ...
            const fm = this.customForm();
            fm.addLabel(`Edit: ${group}`);
            fm.addDropdown(this.tr("deleteUser"), groupInfo.map(u => { const tmp = data.xuid2name(u); return tmp ? tmp : u; }));
            player.sendForm(fm, (pl, dt) => {
                if (dt == null) return this.closeForm(pl);
