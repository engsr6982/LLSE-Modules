...
export default class PermGroup_Form {
     ...
     * @param {Player} player
     */
    closeForm(player: Player) {
        player.tell(this.tr("formClose"));
    }

    inputNull(player: Player) {
     ...
     * @param {Player} player
     */
    editDefaultGroupPerms(player: Player) {
     ...
     * @param {Player} player
     */
    viewPermissionGroups(player: Player, group = {}) {
        const fm = this.customForm();
            ...
            for (const i in group) {
                fm.addLabel(
                    this.tr("listPermissionGroups", {
                        ...
                        perm: group[i].Perm.map((perm: string) => {
                            const tmp = proxyObject.valueToObject(perm);
                            return tmp ? tmp.name : perm;
                        })
                            .join()
                            .replace(/,/g, `§r §l§e| §r`),
                        user: group[i].User.map((user: string) => {
     ...
     * @param {Player} player
     */
    createPermissionGroups(player: Player) {
        const fm = this.customForm();
     ...
     */
    selectPermissionGroup(player: Player, callback: (name: string) => void) {
        const fm = this.simpleForm();
     ...
     * @param {Player} player
     */
    renamePermissionGroup(player: Player) {
        this.selectPermissionGroup(player, (group) => {
            const fm = this.customForm();
            fm.addLabel(`Edit: ${group}`);
     ...
     * @param {Player} player
     */
    searchForUsersPermissionGroup(player: Player) {
        const allPlayer = mc.getOnlinePlayers();
        const fm = this.customForm();
        fm.addDropdown(
            this.tr("addUser.selectPlayer"),
        ...
        player.sendForm(fm, (pl, dt) => {
            ...
            // 辅助数据格式转换函数
            function convertGroups(groups: _UserInGroup) {
                const result = {};
